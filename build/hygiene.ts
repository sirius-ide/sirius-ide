...
const siriusCopyrightLine = ' *  Copyright (c) Clicksora, L.L.C. All rights reserved.';
...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	const productJson = es.through(function (file: VinylFile) {
		// Sirius IDE intentionally ships an extensionsGallery (open-vsx) in product.json,
		// so the upstream check forbidding it is omitted here.
		this.emit('data', file);
	});
